...
export const licenseController = {
  async activate(req: Request, res: Response) {
    ...
    if (!parsed.success) {
      res.status(400).json({ error: "Validation failed", code: "VALIDATION", details: parsed.error.flatten() });
      return;
    }
    const ip = getClientIp(req);
    const userAgent = getUserAgent(req);
    const result = await activateLicense(parsed.data.licenseKey, parsed.data.deviceId, parsed.data.deviceFingerprint, {
      ip,
      userAgent,
    });
    if (!result.ok) {
  ...
  async validate(req: Request, res: Response) {
    ...
    const result = await validateLicense(parsed.data.licenseKey, parsed.data.deviceId, parsed.data.deviceFingerprint);
    if (!result.ok) {
      res.status(400).json({ error: result.reason, code: "LICENSE_INVALID" });
      return;
    }
