 ...
 * GET /api/invoices
 */
invoiceRouter.get("/", requireAuth, async (req: Request, res: Response) => {
  try {
    const tenantId = req.userTenantId;
 ...
 */
invoiceRouter.post("/:invoiceId/receipt", requireAuth, async (req: Request, res: Response) => {
  try {
    const invoiceId = String(req.params.invoiceId);
 ...
 */
invoiceRouter.post("/:invoiceId/qrcode", requireAuth, async (req: Request, res: Response) => {
  try {
    const invoiceId = String(req.params.invoiceId);
    const tenantId = req.userTenantId;
...
invoiceRouter.patch(
  ...
  async (req: Request, res: Response) => {
    try {
      ...
      const newRemainingAmount = Math.max(0, Number(invoice.invoiceTotal) - parsed.paidAmount);
      ...
      if (parsed.paidAmount > 0) {
        const newCustomerBalance = Math.max(0, Number(invoice.previousBalance) - parsed.paidAmount);
 ...
 */
invoiceRouter.get("/:customerId/statement", requireAuth, async (req: Request, res: Response) => {
  try {
